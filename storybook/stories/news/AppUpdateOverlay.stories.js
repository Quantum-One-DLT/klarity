...
storiesOf('News|Overlays', module)
  ...
  .add('Update', ({ locale }: { locale: string }) => {
    ...
    let isWaitingToQuitKlarity = boolean('isWaitingToQuitKlarity', false);
    ...
    if (scenario === 'downloading') {
      ...
      isWaitingToQuitKlarity = boolean('isWaitingToQuitKlarity', false);
      if (isLinux && isWaitingToQuitKlarity)
    ...
    return (
      <AppUpdateOverlay
        ...
        isWaitingToQuitKlarity={isWaitingToQuitKlarity}
        isLinux={isLinux}
        isFlight={isFlight}
        isTestnet={isTestnet}

...
--
data Backend
  = Bcc FilePath -- ^ Bcc SL with the given klarity-bridge.
  deriving (Eq, Show)
...
clusterNetwork Sophie_QA = "sophie_qa"
clusterNetwork Testnet = "testnet"
clusterNetwork Aurum_Purple = "aurum_purple"
...
packageFileName _os cluster ver backend _backendVer build = fromText name <.> ext
  where
    ...
    parts = ["klarity", fromVer ver, lshowText cluster] ++ build'
...
getKlarityVersion :: FilePath -> IO Version
getKlarityVersion packageJSON = L8.readFile (encodeString packageJSON) >>= packageVersion

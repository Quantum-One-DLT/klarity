...
data DarwinConfig = DarwinConfig {
    dcAppNameApp :: Text -- ^ Klarity.app for example
  , dcAppName :: Text -- ^ the Klarity from Klarity.app
  , dcPkgName :: Text -- ^ org.klarity.pkg for example
  , dcDataDir :: Text -- ^ ${HOME}/Library/Application Support/Klarity/qa
  } deriving (Show)
...
main opts@Options{oCodeSigningConfigPath,oSigningConfigPath,oCluster,oBackend,oBuildJob,oOutputDir,oTestInstaller} = do
  ...
  klarityVer <- getKlarityVersion "../package.json"

  let pkg = packageFileName Macos64 oCluster klarityVer oBackend ver oBuildJob
      opkg = oOutputDir </> pkg
...
getBackendVersion (Bcc     bridge) = readBccVersionFile bridge
...
makeComponentRoot Options{oBackend,oCluster} appRoot darwinConfig@DarwinConfig{dcAppName} InstallerConfig{} = do
  ...
  let
    common :: FilePath -> IO ()
    common bridge = do
      -- Executables (from klarity-bridge)
      forM_ ["bcc-launcher" ] $ \f ->
  ...
  case oBackend of
    Bcc bridge -> do
      common bridge
      -- Executables (from klarity-bridge)
      forM_ ["bcc-wallet", "bcc-node", "bcc-cli", "bcc-address" ] $ \f ->
        cp (bridge </> "bin" </> f) (dir </> f)
      forM_ ["config.yaml", "genesis.json", "genesis-cole.json", "genesis-sophie.json", "genesis-aurum.json", "topology.yaml" ] $ \f ->
        cp f (dataDir </> f)
      ...
      void $ chain (encodeString dir) $ fmap tt [dir </> "bcc-launcher", dir </> "bcc-wallet", dir </> "bcc-node", dir </> "bcc-cli", dir </> "bcc-address" ]
...
-- | bcc-sl.klarity-bridge should have a file containing its version.
readBccVersionFile :: FilePath -> IO Text
readBccVersionFile bridge = prefix <$> handle handler (readTextFile verFile)
  where

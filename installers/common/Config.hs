...
optionsParser detectedOS = Options
  ...
  <*> (fromMaybe "klarity" <$> (optional $
      (AppName      <$> optText "appname"             'n' "Application name:  klarity or..")))
...
backendOptionParser = bcc
  where
    bcc = Bcc <$> optPath "bcc" 'S' "Use Bcc backend with given Klarity bridge path"

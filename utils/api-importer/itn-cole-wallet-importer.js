...
const { itnColeMnemonics } = require('./mnemonics');
...
async function main() {
  const shuffledMnemonics = shuffle(itnColeMnemonics);
  ...
  try {
    if (IS_HTTPS) {
      ...
      await Promise.all(sampleSize(shuffledMnemonics, WALLET_COUNT).map((mnemonic, index) => {
        ...
        return request.post(`https://localhost:${API_PORT}/v2/cole-wallets`, payload);
      }));
    } else {
      await Promise.all(sampleSize(shuffledMnemonics, WALLET_COUNT).map((mnemonic, index) => {
        const name = shuffledNames[index];
        const payload = generateImportPayload(mnemonic, name);
        return axios.post(`http://localhost:${API_PORT}/v2/cole-wallets`, payload);
      }));
    }
  } catch (e) {
    console.log(e);

...
# Build/get bcc bridge which is used by make-installer
echo '~~~ Prebuilding bcc bridge'
BCC_BRIDGE=$(nix-build --no-out-link -A klarity-bridge --argstr nodeImplementation bcc)

pushd installers
    echo '~~~ Prebuilding dependencies for bcc-installer, quietly..'
    $nix_shell ../default.nix -A klarity-installer --run true || echo "Prebuild failed!"
    echo '~~~ Building the bcc installer generator..'

    for cluster in ${CLUSTERS}
    do
          echo "~~~ Generating installer for cluster ${cluster}.."

          export KLARITY_CLUSTER="${cluster}"
          APP_NAME="csl-klarity"
          rm -rf "${APP_NAME}"

          echo "Cluster type: bcc"
          BCC_BRIDGE="$(nix-build ../. --no-out-link -A klarity-bridge --argstr nodeImplementation bcc --argstr cluster "${cluster}")"
          BRIDGE_FLAG="--bcc ${BCC_BRIDGE}"

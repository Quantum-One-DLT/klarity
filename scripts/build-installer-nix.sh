...
do
  ...
  nix-build -Q default.nix -A "wrappedBundle" --argstr cluster "${cluster}" --argstr buildNum "$BUILDKITE_BUILD_NUMBER" -o csl-klarity
  if [ -n "${BUILDKITE_JOB_ID:-}" ]; then
    upload_artifacts_public csl-klarity/klarity*.bin
    nix-build -A klarity.cfg  --argstr cluster "${cluster}"

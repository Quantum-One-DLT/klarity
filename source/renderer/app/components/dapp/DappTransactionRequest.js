...
const messages = defineMessages({
  ...
  insufficientBalanceErrorMessage: {
    id: 'dapp.transaction.request.error.notEnoughBcc',
    defaultMessage:
      '!!!This wallet does not contain the minimum amount of {bccBalanceRequired} which is required for delegation to be available. Please select a wallet with <b>a minimum amount of {bccBalanceRequired}</b>.',
    description:
      '"Not enough bcc" error in the dApp transaction request dialog',
  },
});
...
const DappTransactionRequest = observer((props: Props) => {
  ...
  const bccBalanceRequired = bccAmount.plus(transactionFee);
  ...
  const bccAmountErrorMessage = walletsDropdownHasError ? (
    <FormattedHTMLMessage
      {...messages.insufficientBalanceErrorMessage}
      values={{
        bccBalanceRequired: formattedWalletAmount(bccBalanceRequired),
      }}
    />
  ) : null;
  ...
  return (
    ...
    >
      ...
      <AssetsTransactionConfirmation
        ...
        bccError={bccAmountErrorMessage}
      />
      <p className={styles.label}>

...
import bccIcon from '../../../assets/images/bcc-symbol.inline.svg';
...
export default class WalletRow extends Component<Props, WalletRowState> {
  ...
  stakePoolBccSymbolRef: { current: null | HTMLDivElement };

  constructor(props: Props) {
    super(props);

    this.stakePoolFirstTileRef = React.createRef();
    this.stakePoolBccSymbolRef = React.createRef();
  }
  ...
  handleFirstTilePopOverStyle = () => {
    ...
    const { current: bccSymbolDom } = this.stakePoolBccSymbolRef;

    if (!firstTileDom || !bccSymbolDom) {
      if (existingStyle) {
    ...
    const bccSymbolDomRect = bccSymbolDom.getBoundingClientRect();
    const horizontalDelta =
      firstTileDomRect.width / 2 -
      bccSymbolDomRect.width / 2 -
      (bccSymbolDomRect.left - firstTileDomRect.left);
          ...
          <div className={styles.description}>
            {!isRestoring ? (
              <FormattedMessage
                ...
                values={{
                  amount: amount.toFormat(DECIMAL_PLACES_IN_BCC),
                }}
              />
            ) : (
              '-'
                    ...
                    >
                      {delegatedStakePool ? (
                        <div className={styles.stakePoolName}>
                          <div
                            className={styles.activeBccSymbol}
                            ref={this.stakePoolBccSymbolRef}
                          >
                            <SVGInline svg={bccIcon} />
                          </div>

...
export default class AppUpdateStore extends Store {
  ...
  @observable isWaitingToQuitKlarity: boolean = false;
  ...
  _checkNewAppUpdate = async (update: News) => {
    ...
    if (info && data) {
      ...
      // The user reopened Klarity without installing the update
  ...
  _installUpdate = async () => {
    ...
    runInAction(() => {
      this.isWaitingToQuitKlarity = true;
    });
  ...
  _manageQuitAndInstallResponse = ({
    ...
    if (status === statuses.ERROR) {
      ...
      runInAction(() => {
        this.isWaitingToQuitKlarity = false;
      });
      this._setAppAutomaticUpdateFailed();

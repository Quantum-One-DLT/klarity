...
import {
  ...
  SophieTxWitnessSophie,
  SophieTxInputFromUtxo,
  SophieTxOutput,
  SophieTxCert,
  SophieTxWithdrawal,
...
export type ColeEncodeSignedTransactionRequest = {|
  txDataHex: string,
  witnesses: Array<ColeSignedTransactionWitnesses>,
|};

export type ColeSignedTransactionWitnesses = {
  signature: string,
...
const BCC_BCC_APP_POLLING_INTERVAL = 1000;
...
export default class HardwareWalletsStore extends Store {
  @observable selectCoinsRequest: Request<CoinSelectionsResponse> = new Request(
    this.api.bcc.selectCoins
  );
  @observable
  sendMoneyRequest: Request<CreateExternalTransactionResponse> = new Request(
    this.api.bcc.createExternalTransaction
  );
  // @TODO - improve types
  getPublicKeyRequest: Request<any> = new Request(this.api.bcc.getPublicKey);
  // @TODO - improve types
  constructAddressRequest: Request<any> = new Request(
    this.api.bcc.constructAddress
  );
  @observable
  ...
  bccBccAppPollingInterval: ?IntervalID = null;
    ...
    try {
        ...
        if (
          ...
          if (this.isExportKeyAborted) {
            if (isTrezor) {
              ...
              this.bccBccAppPollingInterval = setInterval(
      ...
      if (transportDevice) {
        ...
        if (deviceType === DeviceTypes.TREZOR) {
          ...
          logger.debug('[HW-DEBUG] HWStore - START bcc app poller');
          // Start poller to recognize if Bcc App is launched on device
          const devicePath = transportDevice.path;
          logger.debug(
            '[HW-DEBUG] HWStore - getBccBccApp - from  establishHardwareWalletConnection'
          );
          this.stopBccBccAppFetchPoller();
          this.bccBccAppPollingInterval = setInterval(
            (path) => this.getBccBccApp({ path }),
            BCC_BCC_APP_POLLING_INTERVAL,
            devicePath
          );
        }
  ...
  // Ledger method only
  @action getBccBccApp = async (params: {
    path: ?string,
    walletId?: string,
    address?: ?WalletAddress,
  }) => {
    ...
    logger.debug(
      '[HW-DEBUG] HWStore - START FUNCTION getBccBccApp PARAMS: ',
      { walletId, path, address }
    );

    this.hwDeviceStatus = HwDeviceStatuses.LAUNCHING_BCC_APP;
    try {
      const bccBccApp = await getBccBccAppChannel.request({ path });
      logger.debug(
        '[HW-DEBUG] HWStore - bccBccApp RESPONSE: ',
        bccBccApp
      );
      // Bcc app recognized, stop poller
      this.stopBccBccAppFetchPoller();

      if (bccBccApp) {
        logger.debug('[HW-DEBUG] HWStore - bccBccApp - Set device');

        // Check is Bcc App version supported
        const bccAppVersion = `${bccBccApp.major}.${bccBccApp.minor}.${bccBccApp.patch}`;
        ...
        if (!isValidAppVersion) {
          runInAction(
            'HardwareWalletsStore:: set HW device CONNECTING FAILED - wrong firmware',
            () => {
              this.hwDeviceStatus = HwDeviceStatuses.WRONG_BCC_APP_VERSION;
            }
          );
          throw new Error(
            `Bcc app must be ${MINIMAL_BCC_APP_VERSION} or greater!`
          );
        }
        await this._getExtendedPublicKey(path, walletId, address);
      }
    } catch (error) {
      logger.debug('[HW-DEBUG] HWStore - Bcc app fetching error', {
        error,
      });
      ...
      if (error.code === 'DEVICE_NOT_CONNECTED') {
        // Special case. E.g. device unplugged before bcc app is opened
        ...
        // Special case on Windows where device path changes after opening Bcc app
        // Stop poller and re-initiate connecting state / don't kill devices listener
        this.stopBccBccAppFetchPoller();
  ...
  @action verifyAddress = async (params: {
    ...
    try {
      const derivedAddress = await deriveAddressChannel.request({
        ...
        stakingPathStr: `${SOPHIE_PURPOSE_INDEX}'/${BCC_COIN_TYPE}'/0'/2/0`,
        networkId: isMainnet
          ? HW_SOPHIE_CONFIG.NETWORK.MAINNET.networkId
          : HW_SOPHIE_CONFIG.NETWORK.TESTNET.networkId,
        protocolMagic: isMainnet
          ? HW_SOPHIE_CONFIG.NETWORK.MAINNET.protocolMagic
          : HW_SOPHIE_CONFIG.NETWORK.TESTNET.protocolMagic,
      });
  ...
  @action _getExtendedPublicKey = async (
    ...
    try {
      const extendedPublicKey = await getExtendedPublicKeyChannel.request({
        path: "1852'/1815'/0'", // Sophie 1852 BCC 1815 indicator for account '0'
        isTrezor,
        devicePath,
      });
  ...
  @action _signTransactionTrezor = async (
    ...
    const inputsData = map(inputs, (input) => {
      const sophieTxInput = SophieTxInputFromUtxo(input);
      unsignedTxInputs.push(sophieTxInput);
    ...
    for (const output of outputs) {
      ...
      const sophieTxOutput = SophieTxOutput(output, addressStyle);
      unsignedTxOutputs.push(sophieTxOutput);
    ...
    const _certificatesData = map(certificates, async (certificate) => {
      ...
      const sophieTxCert = SophieTxCert({
        accountAddress,
        pool: certificate.pool,
        type: CERTIFICATE_TYPE[certificate.certificateType],
      });
      unsignedTxCerts.push(sophieTxCert);
    ...
    const fee = formattedAmountToEntropic(flatFee.toString());
    ...
    try {
      const signedTransaction = await signTransactionTrezorChannel.request({
        ...
        networkId: isMainnet
          ...
          : HW_SOPHIE_CONFIG.NETWORK.TESTNET.networkId,
        protocolMagic: isMainnet
          ? HW_SOPHIE_CONFIG.NETWORK.MAINNET.trezorProtocolMagic
          : HW_SOPHIE_CONFIG.NETWORK.TESTNET.trezorProtocolMagic,
      ...
      const unsignedTxWithdrawals =
        withdrawals.length > 0 ? SophieTxWithdrawal(withdrawals) : null;
  ...
  _signWitnesses = async (
    ...
    for (const witness of witnesses) {
      const signedWitness = await this.SophieWitness(witness, xpubHex);
  ...
  SophieWitness = async (
    ...
    if (witnessSignatureHex && publicKey) {
      ...
      return SophieTxWitnessSophie(publicKey, signature);
    }
    return null;
  };
  ...
  initiateTransaction = async (params: {
    ...
    if (deviceType === DeviceTypes.TREZOR && walletId) {
      ...
      logger.debug(
        '[HW-DEBUG] HWStore - getBccBccApp - from  initiateTransaction',
        { devicePath }
      );
      if (walletId) {
        this.stopBccBccAppFetchPoller();
        this.bccBccAppPollingInterval = setInterval(
          (path, wid) => this.getBccBccApp({ path, walletId: wid }),
          BCC_BCC_APP_POLLING_INTERVAL,
          devicePath,
          walletId
        );
  ...
  stopBccBccAppFetchPoller = () => {
    logger.debug('[HW-DEBUG] HWStore - STOP Bcc App poller');
    if (this.bccBccAppPollingInterval) {
      clearInterval(this.bccBccAppPollingInterval);
    }
  };
}

...
import { MAX_BCC_WALLETS_COUNT } from '../config/numbersConfig';
...
export default class WalletsStore extends Store {
  ...
  @observable walletsRequest: Request<Array<Wallet>> = new Request(
    this.api.bcc.getWallets
  );
  @observable accountPublicKeyRequest: Request<string> = new Request(
    this.api.bcc.getAccountPublicKey
  );
  @observable icoPublicKeyRequest: Request<string> = new Request(
    this.api.bcc.getICOPublicKey
  );
  @observable importFromFileRequest: Request<Wallet> = new Request(
    this.api.bcc.importWalletFromFile
  );
  @observable createWalletRequest: Request<Wallet> = new Request(
    this.api.bcc.createWallet
  );
  @observable
  getWalletAddressesRequest: Request<Array<WalletAddress>> = new Request(
    this.api.bcc.getAddresses
  );
  @observable deleteWalletRequest: Request<boolean> = new Request(
    this.api.bcc.deleteWallet
  );
  @observable sendMoneyRequest: Request<WalletTransaction> = new Request(
    this.api.bcc.createTransaction
  );
  @observable getWalletRecoveryPhraseRequest: Request<
    Array<string>
  > = new Request(this.api.bcc.getWalletRecoveryPhrase);
  @observable getWalletCertificateAdditionalMnemonicsRequest: Request<
    Array<string>
  > = new Request(this.api.bcc.getWalletCertificateAdditionalMnemonics);
  @observable getWalletCertificateRecoveryPhraseRequest: Request<
    Array<string>
  > = new Request(this.api.bcc.getWalletCertificateRecoveryPhrase);
  @observable getWalletRecoveryPhraseFromCertificateRequest: Request<
    Array<string>
  > = new Request(this.api.bcc.getWalletRecoveryPhraseFromCertificate);
  @observable restoreKlarityRequest: Request<Wallet> = new Request(
    this.api.bcc.restoreWallet
  );
  @observable restoreLegacyRequest: Request<Wallet> = new Request(
    this.api.bcc.restoreLegacyWallet
  );
  @observable restoreColeRandomWalletRequest: Request<Wallet> = new Request(
    this.api.bcc.restoreColeRandomWallet
  );
  @observable restoreColeIcarusWalletRequest: Request<Wallet> = new Request(
    this.api.bcc.restoreColeIcarusWallet
  );
  @observable restoreColeTrezorWalletRequest: Request<Wallet> = new Request(
    this.api.bcc.restoreColeTrezorWallet
  );
  @observable restoreColeLedgerWalletRequest: Request<Wallet> = new Request(
    this.api.bcc.restoreColeLedgerWallet
  );
  @observable
  transferFundsCalculateFeeRequest: Request<TransferFundsCalculateFeeRequest> = new Request(
    this.api.bcc.transferFundsCalculateFee
  );
  @observable transferFundsRequest: Request<TransferFundsRequest> = new Request(
    this.api.bcc.transferFunds
  );
  @observable createHardwareWalletRequest: Request<Wallet> = new Request(
    this.api.bcc.createHardwareWallet
  );
  ...
  _restoreWalletResetRequests = () => {
    this.restoreKlarityRequest.reset();
    this.restoreColeIcarusWalletRequest.reset();
    this.restoreColeLedgerWalletRequest.reset();
    this.restoreColeRandomWalletRequest.reset();
    this.restoreColeTrezorWalletRequest.reset();
  ...
  _deleteWallet = async (params: { walletId: string, isLegacy: boolean }) => {
    ...
    runInAction('BccWalletsStore::isDeleting set', () => {
      this.isDeleting = true;
    });
    await this._pausePolling();
    const walletToDelete = this.getWalletById(params.walletId);
    if (!walletToDelete) {
      runInAction('BccWalletsStore::isDeleting reset', () => {
    ...
    runInAction('BccWalletsStore::_deleteWallet', () => {
      this.isDeleting = false;
  ...
  _setUndelegateWalletSubmissionSuccess = ({ result }: { result: boolean }) => {
    runInAction(
      'BccWalletsStore::_setUndelegateWalletSubmissionSuccess',
      () => {
  ...
  _restore = async () => {
    ...
    if (
      this.walletKind === WALLET_KINDS.KLARITY &&
      this.walletKindKlarity === WALLET_KLARITY_KINDS.COLE_27_WORD
    ) {
  ...
  @computed get hasMaxWallets(): boolean {
    return this.all.length >= MAX_BCC_WALLETS_COUNT;
  }
  ...
  @computed get restoreRequest(): Request {
    switch (this.walletKind) {
      case WALLET_KINDS.KLARITY:
        if (
          this.walletKindKlarity === WALLET_KLARITY_KINDS.SOPHIE_15_WORD ||
          this.walletKindKlarity === WALLET_KLARITY_KINDS.SOPHIE_24_WORD
        ) {
      ...
      case WALLET_KINDS.HARDWARE:
        if (this.walletKindHardware === WALLET_HARDWARE_KINDS.LEDGER) {
          return this.restoreColeLedgerWalletRequest;
        }
        return this.restoreColeTrezorWalletRequest;
      default:
        return this.restoreKlarityRequest;
    }
  }
  ...
  isValidAddress = async (address: string) => {
    ...
    const validAddressStyles: AddressStyle[] = ['Cole', 'Icarus', 'Sophie'];
    ...
    } else if (isAurumPurple) {
      expectedNetworkTag = AURUM_PURPLE_MAGIC;
    } else if (isSelfnode) {
  ...
  isValidCertificateMnemonic = (mnemonic: string) =>
    this.api.bcc.isValidCertificateMnemonic(mnemonic);
  ...
  @action _pausePolling = async () => {
    ...
    runInAction('BccWalletsStore::_pausePolling', () => {

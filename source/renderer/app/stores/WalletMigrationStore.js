...
import { exportWalletsChannel } from '../ipc/bcc.ipc';
...
export default class WalletMigrationStore extends Store {
  ...
  @observable exportedWallets: Array<ExportedColeWallet> = [];
  ...
  @observable restoreExportedWalletRequest: Request<Wallet> = new Request(
    this.api.bcc.restoreExportedColeWallet
  );
  ...
  getExportedWalletById = (id: string): ?ExportedColeWallet =>
    this.exportedWallets.find((w) => w.id === id);

  getExportedWalletDuplicatesById = (
    id: string,
    index: number
  ): Array<ExportedColeWallet> =>
    this.exportedWallets.filter((w) => w.id === id && w.index !== index);

  getExportedWalletByIndex = (index: number): ?ExportedColeWallet =>
  ...
  @action _restoreWallet = async (exportedWallet: ExportedColeWallet) => {
  ...
  @computed get pendingImportWallets(): Array<ExportedColeWallet> {
    return this.exportedWallets.filter(
      ({ import: { status } }: ExportedColeWallet) =>

...
export default class StakingStore extends Store {
  ...
  bccValue: BigNumber = new BigNumber(82650.15);
  ...
  @action _getSmashSettingsRequest = async () => {
    ...
    // If the server wasn't set, sets it for TBCO
    if (
      !smashServerUrl ||
  ...
  @action showCountdown(): boolean {
    const { isSophiePending } = this.stores.networkStatus;
    return isSophiePending;
  }
  ...
  @action getStakePoolsData = async (isSmash?: boolean) => {
    ...
    if (!isSophieActivated || !isConnected || !isSynced) {
    ...
    try {
      ...
      const stakeInEntropic = parseInt(
        stakeInBigNumber.times(ENTROPICS_PER_BCC),
        10
      );
      await this.stakePoolsRequest.execute(stakeInEntropic).promise;
  ...
  _pollOnSync = () => {
    const { isSynced, isSophieActivated } = this.stores.networkStatus;
    if (isSynced && isSophieActivated) {
      this.getStakePoolsData();
    } else {
      this._resetIsRanking();

...
**2.5 責任：** 当社、ならびにその株主、取締役、役員、従業員、関連会社、および代理人は、トランザクションの確認について、またはユーザーがその暗号通貨もしくはウォレットの復元フレーズ、秘密鍵またはパスワードを紛失するか忘れた場合にそれらを回復させる能力を保証できません。当社は、いかなる理由により生じたかかる損失に責任を負いません。ユーザーは、Klarityまたは当社が行うBCCその他の暗号通貨のトランザクションが遅延するまたは拒否される場合があること、および当社が、この遅延に関連する損失について責任を負わないことを認め、これに合意します。

## 3.特別条件 - インセンティブ付きテストネット、委任、およびステーキング

**3.1 インセンティブ付きテストネット：** 当社は、今後実施されるSophieリリースに含まれる報酬およびインセンティブメカニズムを模すことを意図したテスト環境（「インセンティブ付きテストネット」）で報酬およびインセンティブメカニズムをテストしており、ユーザーによるその試用を可能にしています。ユーザーは、ステークプールにより生成された利益の一定の割合に相当する報酬（「報酬」）と引き換えに自身のBCCステークをユーザー自身のステークプールまたは第三者のステークプールに委任することによってインセンティブ付きテストネットに参加することができます。

**3.2 報酬：** ユーザーが委任により獲得できる報酬額は、ユーザーの参加、ステークプールの利鞘、委任されたBCCの量などさまざまな要素によって決まります。上記の諸要素により、ユーザーが委任により報酬を得られない可能性もあります。報酬は、Bccブロックチェーンの第三エポックの開始以降発生します。インセンティブ付きテストネットの終了に伴い、獲得した報酬はすべてユーザーのKlarityまたはQuantaverseウォレットに移し替えることができます。

**3.3 委任：** ユーザーは、そのステークを当社の複数のステークプールの一つまたは第三者のステークプールに委任することができます。ユーザーは、ステークプールへ委任する量を決定する単独の権利を持ち、随時その参加レベルを上昇・下降させることができます。当社が共有するパフォーマンスメトリクスおよび報酬を含むステークプールに関するあらゆる情報は参考目的であり、正確ではありません。ユーザーは、設定プロセスの時点で更新済みのKlarityまたは更新済みのQuantaverseウォレットにBCCを保有している場合にのみ自身のステークをステークプールに委任することができます。ユーザーは、そのステークを委任する結果として報酬を得るいかなる権利も自動的に得られるものではありません。

**3.4 当社のステークプール：** 当社は、複数のステークプールを運営し、当該ステークプールはKlarityまたはエクスプローラーツールで可視化されます。かかるステークプールを確実に運営するためのコストならびにネットワークおよびサーバー要件は、当社の単独の裁量で決定されるものとします。当社は、ユーザーと共有する報酬額の割合をユーザーのテストネット用Klarityウォレット経由で通知します。報酬は、各エポックの終了時に発生し、ユーザーのテストネット用Klarityウォレットに自動的に表示されます。当社は、当社のステーキングパフォーマンスの詳細をダッシュボードでユーザーに提供します。

**3.5 報酬の還元：** ユーザーは、インセンティブ付きテストネットで報酬を還元することによって得られるのはテスト用BCCに限られ、報酬を実際のBCCに還元するためにはメインネットの公開後にメインネットで同じ還元手続きを繰り返す必要があることを認め、これに合意します。ユーザーは、該当する場合、報酬の還元後に報酬に対するものも含めた適用されるすべての税およびユーザーに適用されるいかなる税もすべて支払う義務を負います。
...
**3.8 保証：** サービスは、ユーザーに「現状有姿」および「利用可能な範囲」ベースで提供され、サービスに関するすべての保証は明示、黙示、法律上の如何を問わず本規約により免責となります。ユーザーは、インセンティブ付きテストネットが正常に動作しない場合があり、エラー、設計上の欠陥その他の不具合が発生し得ることを認識するものとします。ユーザーによるインセンティブ付きテストネットの使用は、ユーザーの責任および裁量で行うものとします。TBCOは、システムにエラーや悪意のある行為が生じた場合、インセンティブ付きテストネットを再起動する、または報酬の全部もしくは一部を取り消す権利を留保します。

**3.9 インセンティブ付きテストネットの終了：** ユーザーは、当社がいつでもその単独の裁量でインセンティブ付きテストネット（またはそのいずれの部分）を変更、撤回、もしくは終了する、またはこれへのアクセスを終了することができることを理解し、これに合意します。ユーザーは、インセンティブ付きテストネットへの参加をいつでも終了することがき、その時点までに得た報酬はインセンティブ付きテストネットが終了するとユーザーのKlarityまたはQuantaverseメインネットウォレットに移されます。
...
**4.4** 製品の特別条件およびポリシーはユーザーによるベータ製品（例：「Klarityウォレットの利用規約」）の使用にも適用することができます。この場合、製品利用規約はユーザーにこれらの規約の一部としてまたは個別に提供され、ユーザーによるベータ製品の使用に適用されます。ベータ製品へのアクセス、そのテストまたは使用を継続することにより、ユーザーは製品の特別条件に拘束されることに同意するものとします。

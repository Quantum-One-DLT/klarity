...
export const formattedWalletAmount = (
  ...
  let formattedAmount = long
    ? new BigNumber(amount).toFormat(DECIMAL_PLACES_IN_BCC)
    : shortNumber(amount);
  ...
  if (withCurrency) {
    formattedAmount = `${formattedAmount} BCC`;
  }
...
export const formattedAmountToEntropic = (amount: string): number =>
  parseInt(formattedAmountToBigNumber(amount).times(ENTROPICS_PER_BCC), 10);

export const formattedEntropicToAmount = (entropic: number): number =>
  formattedAmountToBigNumber(String(entropic))

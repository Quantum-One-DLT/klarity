...
export type SophieTxOutputType = {
  address: string,
...
export type SophieFeeType = {
  fee: number,
  encodeCBOR: Function,
};

export type SophieTtlType = {
  ttl: number,
  encodeCBOR: Function,
};

export type SophieTxWitnessType = {
  publicKey: string,
  signature: Buffer,
  encodeCBOR: Function,
};

export type SophieTxAuxType = {
  getId: Function,
  inputs: Array<SophieTxInputType>,
  outputs: Array<SophieTxOutputType>,
  fee: SophieFeeType,
  ttl: SophieTtlType,
  certs: Array<?Certificate>,
  withdrawals: ?SophieTxWithdrawalsType,
  encodeCBOR: Function,
};

export type SophieTxWithdrawalsType = {
...
// Constructors
export const SophieTxWitnessSophie = (
  publicKey: Buffer,
  signature: Buffer
) => {
...
export const SophieTxInputFromUtxo = (utxoInput: CoinSelectionInput) => {
...
export const SophieTxOutputAssets = (assets: CoinSelectionAssetsType) => {
...
export const SophieTxOutput = (
  output: CoinSelectionOutput,
  addressStyle: AddressStyle
) => {
  const { address, amount, derivationPath, assets } = output;
  const bccCoinQuantity = amount.quantity;
  const coins =
    assets && assets.length > 0
      ? [bccCoinQuantity, SophieTxOutputAssets(assets)]
      : bccCoinQuantity;

  function encodeCBOR(encoder: any) {
    const addressBuff =
      addressStyle === AddressStyles.ADDRESS_SOPHIE
...
export const SophieTxCert = (cert: {
  type: string,
...
export const SophieTxWithdrawal = (
  withdrawals: Array<CoinSelectionWithdrawal>
) => {
  function encodeCBOR(encoder: any) {
...
export const SophieFee = (fee: number) => {
  function encodeCBOR(encoder: any) {
...
export const SophieTtl = (ttl: number) => {
  function encodeCBOR(encoder: any) {
...
export const SophieSignedTransactionStructured = (
  txAux: SophieTxAuxType,
  witnesses: Map<number, SophieTxWitnessType>,
...
export const prepareTxAux = ({
  ...
  txInputs: Array<SophieTxInputType>,
  txOutputs: Array<SophieTxOutputType>,
  txAuxiliaryData?: TxAuxiliaryData,
  ...
  const txFee = SophieFee(fee);
  const txTtl = SophieTtl(ttl);
...
export const prepareBody = (
  unsignedTx: SophieTxAuxType,
  txWitnesses: any, // @TODO - figure out fallback if is Map<number, SophieTxWitnessType> presented as empty array
  txAuxiliaryData: ?CborizedVotingRegistrationMetadata
) => {
  const signedTransactionStructure = SophieSignedTransactionStructured(
    unsignedTx,
    txWitnesses,
    txAuxiliaryData
  );

...
// assigns values to all Klarity CSS variables & returns them
export const createKlarityComponentsTheme = (
  themeParts: PartialThemeParts
): Object => {
  const { colors, fonts } = themeParts;
  const { background, border, error, focus, text } = colors;
  return {
    aboutWindow: {
      ...
      '--theme-about-window-klarity-icon-color': `${text.secondary}`,
      '--theme-about-window-bcc-icon-color': `${text.secondary}`,
    ...
    icon: {
      ...
      '--theme-icon-bcc-summary-wallet-amount-symbol-color': `${text.primary}`,
      ...
      '--theme-icon-connecting-bcc-api-logo-color': `${text.secondary}`,
      '--theme-icon-connecting-bcc-logo-color': `${text.secondary}`,
      '--theme-icon-connecting-klarity-logo-color': `${text.secondary}`,
      ...
      '--theme-icon-syncing-bcc-api-logo-color': `${text.primary}`,
      '--theme-icon-syncing-bcc-logo-color': `${text.primary}`,
      '--theme-icon-syncing-klarity-logo-color': `${text.primary}`,
      '--theme-icon-transactions-bcc-symbol-color': `${text.primary}`,
    ...
    staking: {
      ...
      '--theme-staking-wallet-row-ticker-bcc-icon-fill-color': `${chroma(
        text.primary
      ).alpha(0.5)}`,
...
export const createTheme = (fullThemeParts: CreateThemeParams): Object => {
  ...
  // create react-polymorph & klarity theme, combine into a theme object
  if (colors && !isEmpty(colors) && fonts && !isEmpty(fonts)) {
    klarityTheme = {
      ...createReactPolymorphTheme({ colors, fonts }),
      ...createKlarityComponentsTheme({ colors, fonts }),
    };
  }

  // if user passed theme config, compose its values with klarityTheme object
  if (config && !isEmpty(config)) {
    klarityTheme = updateTheme(klarityTheme, config);
  }

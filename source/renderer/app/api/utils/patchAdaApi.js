...
export default (api: BccApi) => {
  api.getNetworkInfo = async (): Promise<GetNetworkInfoResponse> => {
    logger.debug('BccApi::getNetworkInfo (PATCHED) called');
    try {
      const networkInfo: NetworkInfoResponse = await getNetworkInfo(api.config);
      logger.debug('BccApi::getNetworkInfo (PATCHED) success', { networkInfo });
      ...
      logger.error('BccApi::getNetworkInfo (PATCHED) error', { error });
      throw new ApiError();
    }
  };
  ...
  api.setTestingNewsFeed = (testingNewsFeedData: ?GetNewsResponse) => {
    ...
    const newsFeedItems = map(testingNewsFeedData.items, (item) => {
      return {
        ...
        target: {
          ...item.target,
          klarityVersion: item.target.klarityVersion
            ? packageJsonVersion
            : '',
        },
      };

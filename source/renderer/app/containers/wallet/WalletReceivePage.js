...
export default class WalletReceivePage extends Component<Props, State> {
  ...
  handleGenerateAddress = (passphrase: string) => {
    const { activeWallet } = this;
    if (activeWallet) {
      this.props.actions.addresses.createColeWalletAddress.trigger({
  ...
  render() {
    ...
    const coleWalletAddress = addresses.active ? addresses.active.id : '';
    const isColeWalletAddressUsed = addresses.active
    ...
    return (
      <Fragment>
        <VerticalFlexContainer>
          {isRandom ? (
            <WalletReceiveRandom
              walletAddress={coleWalletAddress}
              isWalletAddressUsed={isColeWalletAddressUsed}
              ...
              isSubmitting={
                addresses.createColeWalletAddressRequest.isExecuting
              }

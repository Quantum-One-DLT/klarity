...
const isOldKlarityLog = (fileName: string) => {
  const appLogNamePosition = fileName.indexOf(ALLOWED_LOGS[0]);

  // if fileName doesn't start with AppLog filename, it's not old klarity log
  ...
  // if extracted string is in -{datetimestamp} format, it's old klarity log
...
const isFileAllowed = (fileName: string) =>
  includes(ALLOWED_LOGS, fileName) || isOldKlarityLog(fileName);

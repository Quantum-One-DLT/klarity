...
import log from 'electron-log-klarity';
...
export const setupLogging = () => {
  const logFilePath = path.join(pubLogsFolderPath, 'Klarity.json');
  ensureDirectoryExists(pubLogsFolderPath);
  rimraf.sync(path.join(pubLogsFolderPath, './Klarity.*'));
  ...
  log.transports.file.format = (message: Object): string => {
    // Debug level logging is recorded as "info" in Klarity log files
...
export const logSystemInfo = (props: LogSystemInfoParams): MessageBody => {
  ...
  const messageBodyParams: ConstructMessageBodyParams = {
    at,
    env,
    ns: ['klarity', `v${klarityVersion}`, `*${network}*`],
    data,
...
export const logStateSnapshot = (
  ...
  const env = `${bccNetwork}:${platform}:${platformVersion}`;
  const messageBodyParams: ConstructMessageBodyParams = {
    at,
    env,
    ...
    sev: 'info',
    thread: '',
    ns: ['klarity', `v${klarityVersion}`, `*${bccNetwork}*`],
    platform,
    platformVersion,
    cpu,
    ram,

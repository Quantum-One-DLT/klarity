...
import {
  BccProcessNameOptions,
  BccNodeImplementationOptions,
  NetworkNameOptions,
} from '../../common/types/bcc-node.types';
...
export const deriveStorageKeys = (
  ...
  PREVIOUS_BCC_PID: `${getNetworkName(network)}-PREVIOUS-BCC-PID`,
});
...
export const deriveProcessNames = (
  ...
  BCC_PROCESS_NAME:
    BccProcessNameOptions[
      isSelfnode
        ? BccNodeImplementationOptions.SELFNODE
        : nodeImplementation
    ][platform] || 'bcc-node',
});
...
export const exportWallets = async (
  ...
  // Prepare Klarity migration data
  try {
...
const prepareMigrationData = async (
  ...
  new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      logger.info('ipcMain: Preparing Klarity Flight migration data...', {
      ...
      logger.info('ipcMain: Preparing Klarity Flight migration data failed', {
        error,
      });
      const { code } = error || {};
      if (code === 'EBUSY') {
        // "EBUSY" error happens on Windows when Klarity mainnet is running during preparation
        // of Klarity Flight wallet migration data as this prevents the files from being copied.
...
const removeMigrationData = async (stateDir: string) => {
  try {
    // Remove migration data dir if it exists
    const migrationDataDirPath = path.join(stateDir, 'migration-data');
    logger.info('ipcMain: Removing Klarity Flight migration data...', {
      migrationDataDirPath,
    });
    await fs.remove(migrationDataDirPath);
    logger.info('ipcMain: Removed Klarity Flight migration data', {
      migrationDataDirPath,
    });
  } catch (error) {
    logger.info('ipcMain: Removing Klarity Flight migration data failed', {
      error,
    });
  }
};

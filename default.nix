...
let
  ...
  pkgs = localLib.tbcoNix.getPkgsDefault { inherit system config; };
  pkgsNative = localLib.tbcoNix.getPkgsDefault {};
  sources = localLib.sources;
  walletPkgs = import "${sources.bcc-wallet}/nix" {};
  # only used for CLI, to be removed when upgraded to next node version
  nodePkgs = import "${sources.bcc-node}/nix" {};
  shellPkgs = (import "${sources.bcc-shell}/nix") {};
  inherit (pkgs.lib) optionalString optional concatStringsSep;
  inherit (pkgs) writeTextFile;
  crossSystem = lib: (crossSystemTable lib).${target} or null;
  # TODO, nsis cant cross-compile with the nixpkgs klarity currently uses
  ...
  packages = self: {
    ...
    bccLib = localLib.tbcoNix.bccLib;
    klarity-bridge = self.bridgeTable.${nodeImplementation};
    ...
    sources = localLib.sources;
    bridgeTable = {
      bcc = self.callPackage ./nix/bcc-bridge.nix {
        bcc-wallet = self.bcc-wallet.bcc-wallet;
        bccWalletPkgs = self.bcc-wallet.pkgs;
      };
    };
    bcc-wallet = import self.sources.bcc-wallet { inherit system; gitrev = self.sources.bcc-wallet.rev; crossSystem = crossSystem walletPkgs.lib; };
    bcc-wallet-native = import self.sources.bcc-wallet { inherit system; gitrev = self.sources.bcc-wallet.rev; };
    bcc-address = (import self.sources.bcc-wallet { inherit system; gitrev = self.sources.bcc-wallet.rev; crossSystem = crossSystem walletPkgs.lib; }).bcc-address;
    mock-token-metadata-server = (import self.sources.bcc-wallet { inherit system; gitrev = self.sources.bcc-wallet.rev; crossSystem = crossSystem walletPkgs.lib; }).mock-token-metadata-server;
    bcc-shell = import self.sources.bcc-shell { inherit system; crossSystem = crossSystem shellPkgs.lib; };
    local-cluster = if cluster == "selfnode" then (import self.sources.bcc-wallet { inherit system; gitrev = self.sources.bcc-wallet.rev; crossSystem = crossSystem walletPkgs.lib; }).local-cluster else null;
    bcc-node-cluster = let
      # Test wallets with known mnemonics
      walletTestGenesisYaml = (self.sources.bcc-wallet + "/lib/sophie/test/data/bcc-node-sophie/genesis.yaml");
      ...
      clusterFunds = import (self.sources.bcc-node + "/nix/supervisord-cluster/initial-funds.nix");
      customConfig = {
        initialFunds = clusterFunds // __foldl' (s: x: s // x) {} initialFundsAttrs;
      };
    in (import self.sources.bcc-node { inherit system customConfig; crossSystem = crossSystem nodePkgs.lib; }).cluster;
    bcc-node = if useLocalNode
                   then (import self.sources.bcc-node { inherit system; crossSystem = crossSystem nodePkgs.lib; }).bcc-node
                   else self.bcc-wallet.bcc-node;
    bcc-cli = if useLocalNode
                   then (import self.sources.bcc-node { inherit system; crossSystem = crossSystem nodePkgs.lib; }).haskellPackages.bcc-cli
                   else self.bcc-wallet.bcc-cli;
    ...
    dlls = pkgs.fetchurl {
      url = "https://s3.eu-central-1.amazonaws.com/klarity-ci-binaries/DLLs.zip";
    ...
    unsignedUnpackedBcc = self.klarity-bridge; # TODO
    unpackedBcc = if dummyInstaller then self.dummyUnpacked else (if needSignedBinaries then self.signedBcc else self.unsignedUnpackedBcc);
    signFile = file: let
      localSigningScript = pkgs.writeScript "signing-script" ''
        ...
        echo $PASS | signcode -spc ${toString signingKeys.spc} -v ${toString signingKeys.pvk} -a sha1 -$ commercial -n "TODO description" -i http://tbco.io -t http://timestamp.verisign.com/scripts/timstamp.dll -tr 10 $FILE | cat
    ...
    signedBcc = let
      copySignedBinaries = let
        signAndCopy = bin: ''
          cp ${self.signFile "${self.unsignedUnpackedBcc}/bin/${bin}"} bin/${bin}
        '';
      in __concatStringsSep "\n" (map signAndCopy self.launcherConfigs.installerConfig.installerWinBinaries);
    in pkgs.runCommand "signed-klarity-bridge" {} ''
      cp -r ${self.unsignedUnpackedBcc} $out
      chmod -R +w $out
      cd $out
      rm bin/*.exe
      ${copySignedBinaries}
    '';
    dummyUnpacked = pkgs.runCommand "dummy-unpacked-bcc" {} ''
      mkdir $out
      cd $out
      touch bcc-launcher.exe bcc-node.exe bcc-x509-certificates.exe log-config-prod.yaml configuration.yaml mainnet-genesis.json
    '';

    nsisFiles = let
      nodeImplementation' = "${nodeImplementation}";
    in pkgs.runCommand "nsis-files" {
      buildInputs = [ self.klarity-installer pkgs.glibcLocales ];
    } ''
      mkdir installers
      cp -vir ${./package.json} package.json
      cd installers

      echo ${self.klarity-bridge.wallet-version} > version
      ...
      mkdir $out
      cp -v klarity.nsi uninstaller.nsi $out/
    ...
    in pkgs.runCommand "win64-installer-${cluster}" {
      buildInputs = [
        self.klarity-installer self.nsis pkgs.unzip pkgs.jq self.yaml2json
      ];
    } ''
      ...
      popd
      cp -v ${self.unpackedBcc}/bin/* .
      cp -v ${self.nsisFiles}/{*.yaml,*.json,klarity.nsi,*.key,*.cert} .
    ...
    signed-windows-installer = let
      backend_version = self.klarity-bridge.wallet-version;
      frontend_version = (builtins.fromJSON (builtins.readFile ./package.json)).version;
      fullName = "klarity-${frontend_version}-${cluster}${buildNumSuffix}.exe"; # must match to packageFileName in make-installer
    ...
    hsKlarityPkgs = self.callPackage ./installers {
      inherit (self) klarity-bridge;
      inherit localLib system;
    };
    klarity-installer = pkgs.haskell.lib.justStaticExecutables self.hsKlarityPkgs.klarity-installer;
    klarity = self.callPackage ./installers/nix/linux.nix {};
    rawapp = self.callPackage ./yarn2nix.nix {
      inherit buildNum;
      api = "bcc";
      apiVersion = self.klarity-bridge.wallet-version;
    ...
    namespaceHelper = pkgs.writeScriptBin "namespaceHelper" ''
      ...
      if [ "x$DEBUG_SHELL" == x ]; then
        exec .${self.nix-bundle.nix-user-chroot}/bin/nix-user-chroot -n ./nix -c -e -m /home:/home -m /etc:/host-etc -m etc:/etc -p DISPLAY -p HOME -p XAUTHORITY -p LANG -p LANGUAGE -p LC_ALL -p LC_MESSAGES -- /nix/var/nix/profiles/profile-${self.linuxClusterBinName}/bin/enter-phase2 klarity
      else
    ...
    desktopItem = pkgs.makeDesktopItem {
      name = "Klarity-${self.linuxClusterBinName}";
      exec = "INSERT_PATH_HERE";
      desktopName = "Klarity ${self.linuxClusterBinName}";
    ...
    postInstall = pkgs.writeScriptBin "post-install" ''
      ...
      export KLARITY_DIR="''${XDG_DATA_HOME}/Klarity/${cluster}"
      mkdir -pv $KLARITY_DIR/Logs/pub

      exec 2>&1 > $KLARITY_DIR/Logs/pub/post-install.log

      echo "in post-install hook"

      cp -f ${self.iconPath.large} $KLARITY_DIR/icon_large.png
      cp -f ${self.iconPath.small} $KLARITY_DIR/icon.png
      cp -Lf ${self.namespaceHelper}/bin/namespaceHelper $KLARITY_DIR/namespaceHelper
      mkdir -pv ~/.local/bin ''${XDG_DATA_HOME}/applications
      cp -Lf ${self.namespaceHelper}/bin/namespaceHelper ~/.local/bin/klarity-${self.linuxClusterBinName}

      cat ${self.desktopItem}/share/applications/Klarity*.desktop | sed \
        -e "s+INSERT_PATH_HERE+''${KLARITY_DIR}/namespaceHelper+g" \
        -e "s+INSERT_ICON_PATH_HERE+''${KLARITY_DIR}/icon_large.png+g" \
        > "''${XDG_DATA_HOME}/applications/Klarity-${self.linuxClusterBinName}.desktop"
    '';
    ...
    preInstall = pkgs.writeText "pre-install" ''
      if grep sse4 /proc/cpuinfo -q; then
        ...
        echo "ERROR: your cpu lacks SSE4 support, bcc will not work"
        exit 1
      fi
    '';
    newBundle = let
      klarity' = self.klarity.override { sandboxed = true; };
    in (import ./installers/nix/nix-installer.nix {
      ...
      installedPackages = [ klarity' self.postInstall self.namespaceHelper klarity'.cfg self.klarity-bridge klarity'.klarity-frontend self.xdg-open ];
      nix-bundle = self.nix-bundle;
    }).installerBundle;
    wrappedBundle = let
      version = (builtins.fromJSON (builtins.readFile ./package.json)).version;
      backend = "bcc-wallet-${nodeImplementation}";
      suffix = if buildNum == null then "" else "-${toString buildNum}";
      fn = "klarity-${version}-${self.linuxClusterBinName}${suffix}.bin";

...
// Create sophie or cole wallets
Given(/^I have (created )?the following (cole )?wallets:$/, async function(mode, _type, table) {
  const type = await getWalletType.call(this, _type);
  const isLegacy = type === 'cole';
...
Given(/^I have a "([^"]*)" (cole )?wallet with funds$/, async function(walletName, _type) {
  const type = await getWalletType.call(this, _type);
  if (type === 'sophie') {
...
Given(/^I have a "([^"]*)" (cole )?wallet$/, async function(walletName, _type) {
  const type = await getWalletType.call(this, _type);
  const isLegacy = type === 'cole';
  if (!isLegacy) {
...
Given(/^I have a "([^"]*)" cole wallet for transfering funds$/, async function(walletName) {
...
Given('I have {int} restored (cole )?wallets', async function(numberOfWallets, _type) {
  const type = await getWalletType.call(this, _type);
  const isLegacy = type === 'cole';
...
Then(
  ...
  async function(walletName, table) {
    ...
    return this.client.waitUntil(async () => {
      const receiverWalletCole = await this.waitAndGetText(
        `.SidebarWalletsMenu_wallets .Wallet_${
          receiverWallet.id
        } .SidebarWalletMenuItem_info`
      );
      return receiverWalletCole === `${expectedData.balance} BCC`;
    }, 60000);
  }
);

Then(
  /^"Cole" wallet badge should be visible in the wallet sidebar$/,
  async function() {

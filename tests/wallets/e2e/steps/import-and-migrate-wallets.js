...
import { MAX_BCC_WALLETS_COUNT } from '../../../../source/renderer/app/config/numbersConfig';
...
Given(/^I have wallet migration enabled/, async function() {
  await this.client.execute(() =>
    klarity.stores.walletMigration._enableTestWalletMigration()
  );
});
...
Given(/^I set (wrong )?import path$/, async function(isWrong) {
  ...
  await this.client.execute((path) => klarity.stores.walletMigration._setFakedImportPath(path), importPath);
});
...
When(/^I should see Klarity State directory as predefined import path$/, async function() {
  ...
  const predefinedStateDirectory = await this.client.execute(() => klarity.stores.walletMigration.defaultExportSourcePath);
...
When(/^I should see wallets properly listed$/, async function() {
  ...
  const exportedWallets = await this.client.execute(() =>
    klarity.stores.walletMigration.exportedWallets
  );
...
When('I should see maximum wallets reached tooltip for wallet with index {int}', async function(walletIndex) {
  ...
  const expectedTooltipText = await this.intl('wallet.select.import.dialog.maxWalletsReachedTooltip', { maxWalletsCount: MAX_BCC_WALLETS_COUNT });

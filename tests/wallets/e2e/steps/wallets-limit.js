...
Given(
  ...
  async function() {
    const wallets = [...Array(MAX_BCC_WALLETS_COUNT)].map((x, i) => ({
      name: `Wallet ${i + 1}`,
      password: 'Secret1234',
    }));
...
When(
  ...
  async function() {
    await this.client.waitUntil(async () => {
      ...
      return walletMenuItems.value.length === MAX_BCC_WALLETS_COUNT;
    });
  }
);
...
When('I delete the last wallet', async function() {
  ...
  await this.client.execute(
    (walletId, isLegacy) => klarity.actions.wallets.deleteWallet.trigger({ walletId, isLegacy }),
  ...
  await this.client.waitUntil(async () => {
    ...
    return wallets.value.length < MAX_BCC_WALLETS_COUNT;
  });
});
...
Then(
  ...
  async function() {
    ...
    expect(disclaimer.replace(/\n/, ' ')).to.equal(
      `You have reached the maximum of ${MAX_BCC_WALLETS_COUNT} wallets. No more wallets can be added.`
    );
  }
);

...
async function prepareNewsOfType(
  ...
  await prepareFakeNews(
    ...
    (news, isRead, done) => {
      const { api } = klarity;
      api.bcc.setTestingNewsFeed(news);
      if (isRead) {
...
export function setNewsFeedIsOpen(client: Object, flag) {
  return client.execute(desiredState => {
    if (klarity.stores.app.newsFeedIsOpen !== desiredState) {
      klarity.actions.app.toggleNewsFeed.trigger();
    }
  }, flag);
}
...
export function resetTestNews(client) {
  return client.executeAsync(done => {
    klarity.api.bcc.setTestingNewsFeed({
      updatedAt: Date.now(),
      items: [],
    });
    klarity.stores.newsFeed.getNews().then(done);
  });
}
...
Given('the newsfeed server is unreachable', async function() {
  this.client.executeAsync(done => {
    klarity.api.bcc.setTestingNewsFeed(null);
    klarity.stores.newsFeed.getNews().then(done);
  });
  this.news = [];
});

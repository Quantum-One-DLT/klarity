...
Before({ tags: '@e2e', timeout: DEFAULT_TIMEOUT * 2 }, async function(testCase) {
  ...
  await this.client.executeAsync(done => {
    const resetBackend = () => {
      if (klarity.stores.networkStatus.isConnected) {
        klarity.stores.wallets
          ._pausePolling()
          .then(() => klarity.stores.wallets.resetWalletsData())
          .then(() => klarity.api.bcc.testReset())
          .then(() => klarity.api.bcc.resetTestOverrides())
          .then(() => klarity.api.localStorage.reset())
          .then(() => klarity.stores.wallets._resumePolling())
          .then(() => klarity.stores.wallets.refreshWalletsData())
          .then(done)
  ...
  await this.client.executeAsync(done => {
    const waitUntilSyncedAndReady = () => {
      if (klarity.stores.networkStatus.isSynced) {
        done();
      } else {
...
After({ tags: '@reconnectApp' }, async function() {
  await this.client.executeAsync(done => {
    klarity.api.bcc.resetTestOverrides();
    klarity.stores.networkStatus

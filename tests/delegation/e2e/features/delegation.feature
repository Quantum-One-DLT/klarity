...
Feature: Wallet Delegation
  ...
  Scenario: "Create Sophie wallet" notification when no Sophie wallets
    Given I am on the Delegation "delegation-center" screen
    Then I should see a "Create a Sophie wallet" notification

  Scenario: Only Sophie wallets are listed on the "Delegation center" screen
    Given I have the following cole wallets:
      | name            |
      | Legacy Wallet 1 |
      | Legacy Wallet 2 |
  ...
  Scenario: "Delegation" wizard displays correct availability
    ...
    Given I have a "Wallet Sender" wallet with funds
    And I send 9 BCC from the "Wallet Sender" wallet to the "Wallet Receiver" wallet
    And I start the wallet delegation process for the "Wallet Receiver" wallet
    Then I should see a "This wallet does not contain the minimum amount of 10 BCC which is required for delegation to be available. Please select a wallet with " message
    Given I close the wizard
    And I send 11 BCC from the "Wallet Sender" wallet to the "Wallet Receiver" wallet

...
Then(/^I should see a "Create a Sophie wallet" notification$/, async function() {
...
Given(/^the "([^"]*)" wallet was delegated to the first Stake Pool$/, async function(walletName) {
  await this.client.waitUntil(async () => {
    const { value: stakePoolsListIsLoaded} = await this.client.executeAsync((done) => done(klarity.stores.staking.stakePools.length > 0));
    return stakePoolsListIsLoaded;
  });
  const data = await this.client.executeAsync((walletName, passphrase, done) => {
    const pool = klarity.stores.staking.stakePools[0];
    const wallet = klarity.stores.wallets.getWalletByName(walletName);
    if (pool && wallet) {
      const { id: stakePoolId } = pool;
      const { id: walletId } = wallet;
      klarity.actions.staking.joinStakePool.trigger({ stakePoolId, walletId, passphrase });
    }
    done(pool);
...
Given(/^the "([^"]*)" wallet was delegated to a Stake Pool with no metadata$/, async function() {
  ...
  await this.client.execute((wallet, stakePool) => {
    klarity.api.bcc.setTestingWallets([wallet]);
    klarity.api.bcc.setTestingStakePools([stakePool]);
...
Given(/^the "([^"]*)" wallet is undelegated$/, async function(wallet) {
  await this.client.executeAsync((walletName, passphrase, done) => {
    ...
    if (pool && wallet) {
      const { id: stakePoolId } = pool;
      const { id: walletId } = wallet;
      klarity.actions.staking.quitStakePool.trigger({ stakePoolId, walletId, passphrase });
    }
    done(pool);
  }, wallet, 'Secret1234');
});
...
Given('I send {int} BCC from the {string} wallet to the {string} wallet', async function(bccAmount, walletFrom, walletTo) {
  await this.client.executeAsync((amount, senderName, receiverName, done) => {
    const walletSender = klarity.stores.wallets.getWalletByName(senderName);
    const walletReceiver = klarity.stores.wallets.getWalletByName(receiverName);

    if (!walletSender || !walletReceiver || !walletSender.id || !walletReceiver.id) {
      return done(new Error(notFoundWalletsErrorMessage));
    }

    return klarity.stores.addresses
      .getAddressesByWalletId(walletReceiver.id)
      .then(addresses => {
        klarity.stores.wallets.sendMoneyRequest.execute({
...
Then(/^I should see a "Loading stake pools" message until the Stake Pools are loaded$/, async function() {
  const { value: stakePoolsLength} = await this.client.execute(() => klarity.stores.staking.stakePools.length);
  if (!stakePoolsLength) {

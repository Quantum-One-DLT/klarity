...
Given(/^the wallets have the following pending delegations:$/, async function(delegationScenariosTable) {
  ...
  await this.client.waitUntil(async () => {
    const stakePools = await this.client.execute(() => klarity.stores.staking.stakePools);
  ...
  const walletsTickersInfo = await this.client.executeAsync((delegationScenarios, done) => {
    ...
    // $FlowFixMe
    klarity.api.bcc.setTestingWallets(modifiedWallets);
    done(walletsTickers);
  }, delegationScenarios);
...
Then(/^the wallets should correctly display the correct stake pool tickers$/, { timeout: 60000 }, async function() {
  ...
  // Waits for the patchBccApi to transform the wallet values
  await this.client.waitUntil(async () => {
...
Then(/^the BCC logo should be displayed as follows:$/, async function(visibilityTable) {
  const visibility = visibilityTable.hashes();
  for (let index = 0; index < visibility.length; index++) {
    const shouldBeVisible = visibility[index].BCC_LOGO === 'visible';
    await this.client.waitForVisible(`.WalletRow_component:nth-child(${index + 1}) .WalletRow_activeBccSymbol`, null, !shouldBeVisible);
  }
});

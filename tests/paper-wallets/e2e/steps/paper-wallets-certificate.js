...
When(/^I click on the print button$/, async function() {
  ...
  await this.client.execute(params => {
    klarity.actions.wallets.generateCertificate.trigger({
...
When(/^I enter paper wallet recovery phrase$/, async function() {
  const fields = await this.client.execute(() => ({
    walletCertificateRecoveryPhrase:
      klarity.stores.wallets.walletCertificateRecoveryPhrase,
    additionalMnemonicWords: klarity.stores.wallets.additionalMnemonicWords,
  }));
...
When(
  /^Bcc explorer link and wallet address should be valid$/,
  async function() {
    const visibleBccExplorerLink = await this.waitAndGetText(
    ...
    const walletCertificateAddress = await this.client.execute(
      () => klarity.stores.wallets.walletCertificateAddress
    );
    const bccExplorerLink = `https://explorer.bcc.org/en/address${
    ...
    expect(bccExplorerLink).to.equal(visibleBccExplorerLink);

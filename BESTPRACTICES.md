...
```javascript
  const name = 'Bcc Entropic';
```

:no_entry_sign: ***Don't***

```javascript
  const name = "Jen Poppins";
  const name = `Jen Poppins`;
```
...
- The Klarity `master` branch is protected by `develop`. Only `develop` is merged into `master`.
...
- To make additions to the Klarity codebase, create a branch based from `develop`.
- There are three main branch types --> `feature`, `chore`, and `fix`.
- A `feature` branch adds a new feature to Klarity that does not yet exist.
...
- YouTrack ID's for Klarity start with the letters `DDW-` + **issue number**.
- A branch suffix is a concise description of the changes contained in the branch.

*Examples*:

* `feature/ddw-41-create-klarity-best-practices-guide`
...
* Add 1 entry per PR and include a URL to the PR at the end of your entry.
  - Example: `- Reduced resource usage ([PR 886](https://github.com/The-Blockchain-Company/klarity/pull/886))`

{ lib, yarn, nodejs, python, api, apiVersion, cluster, buildNum, nukeReferences, fetchzip, klarity, stdenv, win64 ? false, wine64, runCommand, fetchurl, unzip, spacedName, iconPath, launcherConfig, pkgs, python27
, libcap
, libgcrypt
, libgpgerror
, libidn2
...
let
  ...
  hack = writeShellScriptBin "node-gyp" ''
    echo gyp wrapper
    $NIX_BUILD_TOP/klarity/node_modules/electron-rebuild/node_modules/.bin/node-gyp-old "$@" --tarball ${electron-gyp} --nodedir $HOME/.electron-gyp/8.2.2/
  '';
in
yarn2nix.mkYarnPackage {
  name = "klarity-js";
  src = lib.cleanSourceWith { inherit filter; src = ./.; name = "klarity"; };
  API = api;
  ...
  in if win64 then ''
    ...
    ls -ltrh release/win32-x64/Klarity*-win32-x64/
    cp -r release/win32-x64/Klarity*-win32-x64 $out
  ...
  '' else ''
    ...
    ls -ltrha $NIX_BUILD_TOP/klarity/node_modules/
    function dup() {
    ...
    mkdir -p $out/bin $out/share/klarity
    cp -R dist/* $out/share/klarity
    cp ${newPackagePath} $out/share/klarity/package.json
    pushd $out/share/klarity
    ${nukeAllRefs}
    popd
    mkdir -p $out/share/fonts
    ln -sv $out/share/klarity/renderer/assets $out/share/fonts/klarity
    mkdir -pv $out/share/klarity/node_modules
    cp -rv $node_modules/{\@babel,regenerator-runtime,node-fetch,\@trezor,runtypes,parse-uri,randombytes,safe-buffer,bip66,pushdata-bitcoin,bitcoin-ops,typeforce,varuint-bitcoin,bigi,create-hash,merkle-lib,blake2b,nanoassert,blake2b-wasm,bs58check,bs58,base-x,create-hmac,ecurve,wif,ms,keccak,trezor-link,semver-compare,protobufjs-old-fixed-webpack,bytebuffer-old-fixed-webpack,long,object.values,define-properties,object-keys,has,function-bind,es-abstract,has-symbols,json-stable-stringify,tiny-worker,hd-wallet,cashaddrjs,big-integer,queue,inherits,bchaddrjs,cross-fetch,trezor-connect,js-chain-libs-node,bignumber.js,int64-buffer,call-bind,get-intrinsic,cbor-web} $out/share/klarity/node_modules/
    find $out $NIX_BUILD_TOP -name '*.node'

    mkdir -pv $out/share/klarity/build
    cp node_modules/usb/build/Debug/usb_bindings.node $out/share/klarity/build/usb_bindings.node
    cp node_modules/node-hid/build/Debug/HID-hidraw.node $out/share/klarity/build/HID-hidraw.node
    for file in $out/share/klarity/build/usb_bindings.node $out/share/klarity/build/HID-hidraw.node; do
      $STRIP $file
